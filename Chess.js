...
export const FindValidRoad = (from, ColorBoard, chessBoard) => {
  ...
  switch (piece) {
    case 'P': // Pawn
      ...
      if (color === 'b') {
        if (posX === 1) {
          // if this is first step of pawn, pawn can go 2 moves
          maxMoves = 2;
        }
        ...
        //check can eats
        if (
          !!chessBoard[posX + 1][posY - 1] &&
          chessBoard[posX + 1][posY - 1][0] !== color
        ) {
...
const findDiagonal = (posX, posY, ColorBoard, chessBoard, canEats) => {
  ...
  while (tempX >= 0 && tempY >= 0) {
    // check cheo len trai x giam, y giam
...
export const Check = (chessBoard, kingPosX, kingPosY, KingColor) => {
  ...
  while (tempX >= 0) {
    ...
    if (value) {
      if (value[0] !== KingColor && (value[1] === 'R' || value[1] === 'Q')) {
    ...
    if (value) {
      if (value[0] !== KingColor) {
        if (value[1] === 'Q' || value[1] === 'B') {
          ...
          // king
          if (kingPosX - tempX === 1) {
            // king1 just can check king2 in range around one square
